...
fn test_public_api_single_thread() {
    ...
    let id1 = generator
        .generate_id()
        .expect("Failed to generate first ID");
    let id2 = generator
        .generate_id()
        .expect("Failed to generate second ID");

    // 3. We verify basic sequential logic through the public boundary
    assert!(
        id2 > id1,
        "Subsequent IDs should be strictly greater in the same millisecond"
    );
}
    ...
    for handle in handles {
        ...
        for id in node_ids {
            assert!(
                ...
                "GLOBAL COLLISION! Two different nodes generated the exact same ID: {}",
                id
            );
        }
    }
